...
type Section = "概览" | "经营分析" | "良率分析" | "打印物品" | "耗材库存" | "耗材卷同步" | "订单" | "打印队列" | "生产明细" | "文件资产" | "设备管理" | "系统中心";
...
export default function Home() {
  ...
  function openCreate() { if(["生产明细","文件资产","设备管理","耗材卷同步","经营分析","良率分析","系统中心"].includes(section)) return; setModal(section === "概览" ? "job" : entityBySection[section]); }
  ...
  async function updateMaterialCost(material:Material){const value=window.prompt(`设置 ${material.material} ${material.color} 的采购成本（元/kg）`,String(material.costPerKg||0));if(value===null)return;const response=await fetch("/api/workspace",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({entity:"material",id:material.id,costPerKg:Number(value)})});if(response.ok){toast("耗材成本已更新");await loadData();}else toast("成本更新失败");}
  ...
  return <main className="shell">
    <aside className="sidebar">
      ...
      <nav><p className="nav-title">工作空间</p><a className="main-nav-link" href="/team"><span>♙</span>员工与权限</a><a className="main-nav-link" href="/fleet"><span>▣</span>打印机中枢</a>{nav.map(item => <button key={item.label} className={section===item.label?"nav-active":""} onClick={() => setSection(item.label)}><span>{item.mark}</span>{item.label}{item.label==="打印队列"&&<b>{waiting}</b>}</button>)}<button className={section==="系统中心"?"nav-active":""} onClick={() => setSection("系统中心")}><span>⚙</span>系统中心</button></nav>
      <div className="sidebar-bottom"><div className="system-state"><i/> {loading?"正在同步数据":"数据已同步"}</div><div className="profile"><span>郑</span><div><strong>管理员</strong><small>私有工作区</small></div><em>•••</em></div></div>
    </aside>
    <section className="content">
      ...
      <div className="workspace">
        ...
        {section === "概览" ? <Dashboard data={data} metrics={{printing,waiting,completed,alerts}} onNavigate={setSection} onAdvance={advanceJob}/> : section === "经营分析" ? <Analytics toast={toast}/> : section === "良率分析" ? <QualityAnalytics toast={toast}/> : section === "系统中心" ? <SystemCenter toast={toast}/> : section === "生产明细" ? <ProductionDetails data={data} toast={toast} onWorkspaceChanged={loadData}/> : section === "文件资产" ? <FileAssets data={data} toast={toast}/> : section === "设备管理" ? <PrinterManager toast={toast}/> : section === "耗材卷同步" ? <SpoolmanInventory toast={toast}/> : <Management section={section} filtered={filtered} onDelete={remove} onAction={runJobAction} onCost={updateMaterialCost}/>}
      </div>
    </section>
...
function Management({section,filtered,onDelete,onAction,onCost}:{section:"打印物品"|"耗材库存"|"订单"|"打印队列";filtered:{items:Item[];materials:Material[];orders:Order[];jobs:Job[]};onDelete:(e:Entity,id:number)=>void;onAction:(j:Job,a:string)=>void;onCost:(m:Material)=>void}) {
  const configs = {
    ...
    "耗材库存": { eyebrow:"MATERIAL BATCHES", note:"按卷材批次跟踪重量、预警线和采购成本", heads:["材料","颜色","品牌","当前余量","采购成本","预警线","操作"], rows:filtered.materials.map(x=>[x.material,x.color,x.brand||"—",`${x.remainingGrams} / ${x.initialGrams} g`,`¥${x.costPerKg||0}/kg`,`${x.lowStockGrams} g`,<span className="row-actions" key="d"><button onClick={()=>onCost(x)}>改成本</button><button className="danger-link" onClick={()=>onDelete("material",x.id)}>删除</button></span>]) },
...
type SystemData={health:{status:string;offlinePrinters:number;pendingCommands:number;failedCommands:number;lowStock:number;checkedAt:string};alerts:Array<{level:string;title:string;detail:string}>;audit:Array<{createdAt:string;source:string;action:string;detail:string}>};
function SystemCenter({toast}:{toast:(m:string)=>void}){const [data,setData]=useState<SystemData|null>(null);async function load(){const response=await fetch("/api/system",{cache:"no-store"});const result=await response.json();if(response.ok)setData(result);else toast(result.error||"系统检查失败");}useEffect(()=>{void load();const timer=window.setInterval(()=>void load(),30000);return()=>window.clearInterval(timer);},[]);if(!data)return <div className="empty-state">正在执行系统健康检查…</div>;const h=data.health;return <section><div className={`health-banner ${h.status==="正常"?"healthy":"attention"}`}><div><small>SYSTEM HEALTH</small><h2>{h.status}</h2><p>最后检查：{new Date(h.checkedAt).toLocaleString("zh-CN")} · 每 30 秒自动刷新</p></div><div><strong>{h.offlinePrinters}</strong><span>离线设备</span></div><div><strong>{h.pendingCommands}</strong><span>超时命令</span></div><div><strong>{h.failedCommands}</strong><span>失败命令</span></div><div><strong>{h.lowStock}</strong><span>库存告警</span></div><a className="primary" href="/api/system?format=backup">下载完整备份</a></div><div className="system-grid"><div className="panel"><PanelHead eyebrow="ACTIVE ALERTS" title="异常告警" action="刷新 ↻" onClick={()=>void load()}/><div className="alert-list">{data.alerts.map((alert,index)=><article className={alert.level} key={`${alert.title}-${index}`}><i>{alert.level==="danger"?"!":"△"}</i><div><strong>{alert.title}</strong><p>{alert.detail}</p></div></article>)}{data.alerts.length===0&&<div className="empty-state">当前没有异常告警。</div>}</div></div><div className="panel"><PanelHead eyebrow="RECOVERY" title="备份与恢复说明"/><div className="recovery-copy"><p>备份包含订单、任务、库存、BOM、文件元数据、设备状态、命令与审计记录。</p><p>设备连接令牌哈希不会导出，避免备份文件泄露后被用于连接设备。</p><p>请定期下载并保存到加密磁盘；恢复建议在正式迁移时由管理员离线执行。</p></div></div></div><div className="panel audit-panel"><PanelHead eyebrow="AUDIT TRAIL" title="关键操作审计 · 最近 100 条"/><div className="table-wrap"><table><thead><tr><th>时间</th><th>来源</th><th>操作</th><th>详情</th></tr></thead><tbody>{data.audit.map((row,index)=><tr key={`${row.createdAt}-${index}`}><td>{row.createdAt}</td><td>{row.source}</td><td>{row.action}</td><td>{row.detail}</td></tr>)}</tbody></table></div></div></section>}

type QualityRow={name:string;total:number;completed:number;failed:number};type AnalyticsData={summary:{revenue:number;materialCost:number;machineCost:number;energyCost:number;laborCost:number;overheadCost:number;totalCost:number;grossProfit:number;margin:number;successRate:number;utilization:number;completed:number;reworks:number};settings:{electricityRate:number;laborRate:number;laborMinutesPerJob:number;overheadPercent:number};orders:Array<{orderNo:string;customer:string;status:string;revenue:number;jobs:number;completedJobs:number}>;trends:Array<{day:string;completed:number}>;byPrinter:QualityRow[];byItem:QualityRow[];reasons:Array<{reason:string;count:number}>};
function Analytics({toast}:{toast:(m:string)=>void}){
  const [data,setData]=useState<AnalyticsData|null>(null);
  const load=()=>fetch("/api/analytics",{cache:"no-store"}).then(response=>response.json().then(value=>({ok:response.ok,value}))).then(({ok,value})=>ok?setData(value):toast(value.error||"分析数据读取失败")).catch(()=>toast("分析数据读取失败"));
  useEffect(()=>{void load()},[]);
  async function saveSettings(event:FormEvent<HTMLFormElement>){event.preventDefault();const response=await fetch("/api/cost-settings",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});if(response.ok){toast("成本参数已更新");await load()}else toast("成本参数更新失败")}
  if(!data)return <div className="empty-state">正在计算经营数据…</div>;
  const s=data.summary,money=(value:number)=>`¥${value.toFixed(2)}`,max=Math.max(1,...data.trends.map(x=>x.completed));
  return <section><div className="analytics-metrics"><Metric label="订单收入" value={money(s.revenue)} unit="累计报价" delta="订单明细汇总" accent="green"/><Metric label="生产总成本" value={money(s.totalCost)} unit="五项成本合计" delta={`材料 ${money(s.materialCost)}`} accent="orange"/><Metric label="预计毛利" value={money(s.grossProfit)} unit={`${s.margin.toFixed(1)}% 毛利率`} delta="收入减完整生产成本" accent="blue"/><Metric label="打印成功率" value={`${s.successRate.toFixed(1)}%`} unit={`${s.completed} 个完成任务`} delta={`设备利用率 ${s.utilization.toFixed(1)}%`} accent="red"/></div><div className="analytics-grid"><div className="panel"><PanelHead eyebrow="7 DAY OUTPUT" title="近 7 日完成趋势"/><div className="trend-chart">{data.trends.map(x=><div key={x.day}><span style={{height:`${Math.max(6,x.completed/max*100)}%`}} title={`${x.completed} 个任务`}/><b>{x.completed}</b><small>{x.day.slice(5)}</small></div>)}</div></div><div className="panel"><PanelHead eyebrow="COST STRUCTURE" title="完整成本结构"/><div className="cost-stack"><div><span>材料成本</span><b>{money(s.materialCost)}</b></div><div><span>设备折旧/工时</span><b>{money(s.machineCost)}</b></div><div><span>电费</span><b>{money(s.energyCost)}</b></div><div><span>人工</span><b>{money(s.laborCost)}</b></div><div><span>管理损耗</span><b>{money(s.overheadCost)}</b></div><div className="total"><span>合计</span><b>{money(s.totalCost)}</b></div></div></div></div><form className="panel cost-settings" onSubmit={saveSettings}><PanelHead eyebrow="COST PARAMETERS" title="成本参数"/><div><Field name="electricityRate" label="电价（元/kWh）" type="number" defaultValue={String(data.settings.electricityRate||0.8)}/><Field name="laborRate" label="人工时薪（元/小时）" type="number" defaultValue={String(data.settings.laborRate||0)}/><Field name="laborMinutesPerJob" label="每单人工分钟" type="number" defaultValue={String(data.settings.laborMinutesPerJob||0)}/><Field name="overheadPercent" label="管理损耗（%）" type="number" defaultValue={String(data.settings.overheadPercent||0)}/><button className="primary">保存成本参数</button></div><p>电费按设备默认功率 1000W × 实际打印时长计算；接入打印机后可继续细化每台设备功率。</p></form><div className="panel analytics-orders"><PanelHead eyebrow="ORDER PROFITABILITY" title="订单收入与生产进度"/><div className="table-wrap"><table><thead><tr><th>订单</th><th>客户</th><th>状态</th><th>收入</th><th>打印任务</th><th>已完成</th></tr></thead><tbody>{data.orders.map(order=><tr key={order.orderNo}><td>{order.orderNo}</td><td>{order.customer}</td><td><span className="badge">{order.status}</span></td><td>{money(Number(order.revenue))}</td><td>{order.jobs}</td><td>{order.completedJobs}</td></tr>)}</tbody></table></div></div></section>
}
...
function SpoolmanInventory({toast}:{toast:(m:string)=>void}){
  const [spools,setSpools]=useState<SyncedSpool[]>([]);const [printers,setPrinters]=useState<SpoolPrinter[]>([]);const [loading,setLoading]=useState(true);
  async function load(){setLoading(true);const response=await fetch("/api/spools",{cache:"no-store"});const result=await response.json();setLoading(false);if(response.ok){setSpools(result.spools);setPrinters(result.printers);}else toast(result.error||"耗材卷读取失败");}
  useEffect(()=>{void load();},[]);
  async function mount(spool:SyncedSpool){if(!printers.length){toast("请先添加打印机");return;}const menu=printers.map((p,i)=>`${i+1}. ${p.name}${p.activeSpoolExternalId?`（当前 #${p.activeSpoolExternalId}）`:""}`).join("\n");const choice=Number(window.prompt(`将耗材卷 #${spool.externalId} 挂载到：\n${menu}`,"1"));const printer=printers[choice-1];if(!printer)return;const response=await fetch("/api/spools",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({printerId:printer.id,spoolId:spool.externalId})});const result=await response.json();if(!response.ok){toast(result.error||"挂载失败");return;}toast(`耗材卷 #${spool.externalId} 已挂载到 ${printer.name}`);await load();}
  const active=spools.filter(s=>!s.archived);const total=active.reduce((sum,s)=>sum+(s.remainingWeight||0),0);
  return <section><div className="spool-summary"><div><small>SPOOLMAN STATUS</small><strong>{active.length}</strong><span>可用耗材卷</span></div><div><small>REMAINING</small><strong>{Math.round(total)}g</strong><span>同步剩余重量</span></div><div><small>MOUNTED</small><strong>{printers.filter(p=>p.activeSpoolExternalId).length}</strong><span>已挂载打印机</span></div></div><div className="panel"><PanelHead eyebrow="SPOOLMAN INVENTORY" title="耗材卷库存" action={loading?"同步中…":"刷新 ↻"} onClick={()=>void load()}/><div className="spool-grid">{spools.map(spool=>{const capacity=spool.initialWeight||((spool.remainingWeight||0)+(spool.usedWeight||0));const percent=capacity?Math.max(0,Math.min(100,(spool.remainingWeight||0)/capacity*100)):0;const mounted=printers.filter(p=>p.activeSpoolExternalId===spool.externalId);return <article className={`spool-card ${spool.archived?"archived":""}`} key={spool.id}><div className="spool-ring" style={{"--spool-color":spool.colorHex?`#${spool.colorHex.replace("#","")}`:"#8c9b95","--spool-level":`${percent}%`} as React.CSSProperties}><i/></div><div><div className="spool-title"><strong>#{spool.externalId} · {spool.filamentName||spool.material||"未命名耗材"}</strong><span>{spool.archived?"已归档":percent<20?"低库存":"可用"}</span></div><p>{spool.vendor||"未知厂商"} · {spool.material||"未知材质"} · {spool.location||"未设置位置"}</p><div className="spool-weight"><b>{Math.round(spool.remainingWeight||0)}g</b><span>剩余 / {Math.round(capacity||0)}g</span></div><div className="spool-bar"><i style={{width:`${percent}%`}}/></div><small>批次 {spool.lotNr||"--"}{mounted.length?` · 已挂载 ${mounted.map(p=>p.name).join("、")}`:""}</small>{!spool.archived&&<button className="spool-mount" onClick={()=>mount(spool)}>挂载到打印机</button>}</div></article>})}{!loading&&spools.length===0&&<div className="empty-state">尚未收到 Spoolman 数据。请在本地代理设置 SPOOLMAN_URL。</div>}</div></div></section>}
...
function FileAssets({data,toast}:{data:WorkspaceData;toast:(m:string)=>void}){
  ...
  return <section className="file-layout"><div className="panel file-upload"><div className="management-hero"><small>MODEL REPOSITORY</small><h2>上传打印文件</h2><p>支持 STL、3MF、G-code 和产品预览图片，单文件最大 100MB</p></div><form className="detail-form" onSubmit={submit}><label className="file-picker"><span>选择文件</span><input type="file" name="file" accept=".stl,.3mf,.gcode,.gco,.png,.jpg,.jpeg,.webp" required/></label><label><span>关联物品</span><select name="itemId"><option value="">暂不关联</option>{data.items.map(x=><option key={x.id} value={x.id}>{x.sku} · {x.name}</option>)}</select></label><Field name="version" label="版本" defaultValue="v1"/><Field name="printerProfile" label="打印机配置" placeholder="例如：Bambu X1C 0.4mm"/><Field name="layerHeight" label="层高（mm）" type="number"/><Field name="infillPercent" label="填充率（%）" type="number"/><Field name="estimatedMinutes" label="预计时长（分钟）" type="number"/><Field name="notes" label="备注" placeholder="切片器、喷嘴或变更说明"/><button className="primary detail-save" disabled={uploading}>{uploading?"上传中…":"上传并保存"}</button></form></div>
    <div className="panel file-library"><PanelHead eyebrow="FILES & VERSIONS" title="文件库" action="刷新 ↻" onClick={()=>void load()}/><div className="file-cards">{files.map(file=><article className="file-card" key={file.id}><div className={`file-kind ${file.kind==="图片"?"image":""}`}>{file.kind==="图片"?"▧":file.kind==="G-code"?"G":"3D"}</div><div className="file-info"><div><strong>{file.filename}</strong><span>{file.kind} · {file.version}</span></div><p>{file.itemName||"未关联物品"}　·　{size(file.sizeBytes)}</p><small>{file.printerProfile||"未设置打印机配置"}{file.layerHeight?` · ${file.layerHeight}mm`:""}{file.infillPercent?` · 填充 ${file.infillPercent}%`:""}</small></div><div className="file-actions">{["G-code","3MF"].includes(file.kind)&&<button onClick={()=>dispatch(file)}>发送并打印</button>}<a href={`/api/files?download=${file.id}`}>下载</a><button onClick={()=>remove(file.id)}>删除</button></div></article>)}{files.length===0&&<div className="empty-state">还没有文件，先上传一个模型或 G-code。</div>}</div></div></section>;
}

type AmsSlot={id:number;amsUnit:number;trayIndex:number;material:string;colorHex:string;remainingPercent:number|null;active:boolean};
type Printer={id:number;name:string;model:string;technology:string;location:string;nozzleDiameter:number;buildVolume:string;status:string;totalHours:number;hourlyRate:number;maintenanceDueAt:string|null;notes:string;connectorType:string;connectionState:string;lastSeenAt:string|null;nozzleTemp:number|null;bedTemp:number|null;currentFile:string|null;remoteProgress:number|null;amsSlots?:AmsSlot[]};
function PrinterManager({toast}:{toast:(m:string)=>void}){
  ...
  async function rate(printer:Printer){const value=window.prompt(`设置 ${printer.name} 的设备成本（元/小时）`,String(printer.hourlyRate||0));if(value===null)return;const response=await fetch("/api/printers",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:printer.id,hourlyRate:Number(value)})});if(response.ok){toast("设备工时成本已更新");await load();}else toast("成本更新失败");}
  async function connect(printer:Printer){const connectorType=window.prompt("连接类型：bambu_lan、moonraker 或 octoprint",printer.connectorType==="manual"?"bambu_lan":printer.connectorType);if(!connectorType)return;const response=await fetch("/api/printers",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:printer.id,action:"rotateToken",connectorType})});const result=await response.json();if(!response.ok){toast(result.error||"生成令牌失败");return;}await navigator.clipboard.writeText(result.token);window.alert(`代理令牌已复制。请立即保存，关闭后将无法再次查看：\n\n${result.token}${connectorType==="bambu_lan"?"\n\n本地 Agent 还需设置 BAMBU_HOST、BAMBU_SERIAL、BAMBU_ACCESS_CODE。":""}`);await load();}
...
function CreateModal({entity,data,onClose,onSaved}:{entity:Entity;data:WorkspaceData;onClose:()=>void;onSaved:()=>void}) {
  ...
  return <div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><form className="record-modal" onSubmit={submit}><div className="modal-head"><div><small>NEW RECORD</small><h2>新建{titles[entity]}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid">
    ...
    {entity==="material"&&<><Field name="material" label="材料类型" placeholder="PLA"/><Field name="color" label="颜色" placeholder="哑光白"/><Field name="brand" label="品牌" placeholder="eSUN"/><Field name="initialGrams" label="初始重量（g）" type="number" defaultValue="1000"/><Field name="remainingGrams" label="当前余量（g）" type="number" defaultValue="1000"/><Field name="lowStockGrams" label="预警线（g）" type="number" defaultValue="200"/><Field name="costPerKg" label="采购成本（元/kg）" type="number" defaultValue="0"/></>}
...
function Field({name,label,placeholder,type="text",defaultValue}:{name:string;label:string;placeholder?:string;type?:string;defaultValue?:string}) { const optional=["category","brand","note","notes","printerProfile","layerHeight","infillPercent","estimatedMinutes","model","location","buildVolume","totalHours","hourlyRate","costPerKg","maintenanceDueAt"].includes(name); return <label><span>{label}</span><input name={name} type={type} step={type==="number"?"any":undefined} placeholder={placeholder} defaultValue={defaultValue} required={!optional}/></label>; }
