...
function Dashboard({
  ...
  const [workbench,setWorkbench]=useState<{metrics:{cashGap30Cents:number;projectedCash30Cents:number;overdueReceivableCents:number;overduePayableCents:number;lowStock:number;criticalAlerts:number;orderRisks:number;unsettledPrints:number};priorities:Array<{key:string;tone:string;title:string;detail:string;href:string}>}|null>(null);
  ...
  return (
    <>
      ...
      {workbench&&<section className="business-control-strip"><a href="/cashflow" className={workbench.metrics.cashGap30Cents?"danger":""}><span>30天预计资金</span><strong>{new Intl.NumberFormat("zh-CN",{style:"currency",currency:"MYR"}).format(workbench.metrics.projectedCash30Cents/100)}</strong><small>{workbench.metrics.cashGap30Cents?`缺口 ${(workbench.metrics.cashGap30Cents/100).toFixed(2)} MYR`:"高于安全储备"}</small></a><a href="/receivables" className={workbench.metrics.overdueReceivableCents?"warning":""}><span>逾期应收</span><strong>{new Intl.NumberFormat("zh-CN",{style:"currency",currency:"MYR"}).format(workbench.metrics.overdueReceivableCents/100)}</strong><small>优先跟进回款</small></a><a href="/payables" className={workbench.metrics.overduePayableCents?"warning":""}><span>逾期应付</span><strong>{new Intl.NumberFormat("zh-CN",{style:"currency",currency:"MYR"}).format(workbench.metrics.overduePayableCents/100)}</strong><small>检查付款安排</small></a><a href="/alerts" className={workbench.metrics.criticalAlerts?"danger":""}><span>关键运行风险</span><strong>{workbench.metrics.criticalAlerts+workbench.metrics.orderRisks+workbench.metrics.unsettledPrints}</strong><small>告警＋订单＋待结算</small></a></section>}
