...
export async function GET(){
  ...
  const [summary,catalog,spools,locations,printers,positions,bindings,movements,usage,unboundSlots,transit]=await Promise.all([
    ...
    db.prepare(`SELECT s.id,s.spool_code spoolCode,s.state,s.initial_net_grams initialNetGrams,s.remaining_net_grams remainingNetGrams,s.tare_grams tareGrams,s.last_gross_grams lastGrossGrams,s.rfid_uid rfidUid,s.last_weighed_at lastWeighedAt,s.updated_at updatedAt,
      ...
      COALESCE(pl.unit_cost_cents_per_kg,0) unitCostCentsPerKg,COALESCE(pl.lot_no,'') lotNo,(SELECT MAX(m.occurred_at) FROM material_spool_movements m WHERE m.organization_id=s.organization_id AND m.spool_id=s.id AND m.movement_type IN ('consume','loss','weigh')) lastUsedAt
      FROM material_spools s JOIN material_catalog_items c ON c.id=s.catalog_item_id AND c.organization_id=s.organization_id JOIN inventory_locations_v2 l ON l.id=s.current_location_id AND l.organization_id=s.organization_id LEFT JOIN material_purchase_lots pl ON pl.id=s.purchase_lot_id AND pl.organization_id=s.organization_id
...
export async function POST(request:Request){
  ...
  try{
    if(action==="updateSpool"){
      const spoolId=id(body.spoolId),spoolCode=text(body.spoolCode,80).toUpperCase(),catalogItemId=id(body.catalogItemId),locationId=id(body.locationId),lotNo=text(body.lotNo,80).toUpperCase(),cost=Math.max(0,Math.round(Number(body.costPerKg||0)*100));if(!spoolId||!spoolCode||!catalogItemId||!locationId||!lotNo)return fail("请完整填写卷码、耗材目录、采购批次和仓位");
      const spool=await db.prepare("SELECT s.id,s.state,s.purchase_lot_id lotId,EXISTS(SELECT 1 FROM print_material_usage_lines u WHERE u.organization_id=s.organization_id AND u.spool_id=s.id) hasUsage FROM material_spools s WHERE s.id=? AND s.organization_id=?").bind(spoolId,org).first<{id:number;state:string;lotId:number;hasUsage:number}>();if(!spool)return fail("实体卷不存在",404);if(spool.state==="in_use")return fail("使用中的耗材请先从打印机退库后再编辑",409);if(spool.hasUsage&&Number(body.catalogItemId)!==catalogItemId)return fail("已有打印用量的实体卷不能更换耗材身份",409);
      const catalog=await db.prepare("SELECT id FROM material_catalog_items WHERE id=? AND organization_id=?").bind(catalogItemId,org).first(),location=await db.prepare("SELECT id FROM inventory_locations_v2 WHERE id=? AND organization_id=? AND active=1").bind(locationId,org).first();if(!catalog||!location)return fail("耗材目录或仓位不存在",404);
      await db.batch([db.prepare("UPDATE material_spools SET spool_code=?,catalog_item_id=?,current_location_id=?,updated_at=? WHERE id=? AND organization_id=?").bind(spoolCode,catalogItemId,locationId,now,spoolId,org),db.prepare("UPDATE material_purchase_lots SET lot_no=?,catalog_item_id=?,unit_cost_cents_per_kg=? WHERE id=? AND organization_id=?").bind(lotNo,catalogItemId,cost,spool.lotId,org)]);await recordAudit(context,"inventory_v2.spool.updated","material_spool",String(spoolId),{spoolCode,catalogItemId,locationId,lotNo,cost});return Response.json({id:spoolId});
    }
    if(action==="voidSpool"){
      const spoolId=id(body.spoolId),reason=text(body.reason,300);if(!spoolId||reason.length<4)return fail("请填写至少 4 个字的撤销或报废原因");
      const spool=await db.prepare("SELECT s.id,s.state,s.remaining_net_grams remaining,s.current_location_id locationId,EXISTS(SELECT 1 FROM spool_bindings b WHERE b.organization_id=s.organization_id AND b.spool_id=s.id AND b.status='active') bound,EXISTS(SELECT 1 FROM print_material_usage_lines u WHERE u.organization_id=s.organization_id AND u.spool_id=s.id) hasUsage FROM material_spools s WHERE s.id=? AND s.organization_id=?").bind(spoolId,org).first<{id:number;state:string;remaining:number;locationId:number;bound:number;hasUsage:number}>();if(!spool)return fail("实体卷不存在",404);if(spool.bound)return fail("实体卷仍绑定打印机，请先退库",409);if(["empty","scrapped"].includes(spool.state))return fail("实体卷已经退出有效库存",409);
      const key=`void:${crypto.randomUUID()}`;await db.batch([db.prepare("UPDATE material_spools SET state='scrapped',remaining_net_grams=0,updated_at=? WHERE id=? AND organization_id=?").bind(now,spoolId,org),db.prepare("INSERT INTO material_spool_movements(organization_id,spool_id,movement_type,from_location_id,net_grams_delta,idempotency_key,operator_email,note) VALUES(?,?,'scrap',?,?,?,?,?)").bind(org,spoolId,spool.locationId,-spool.remaining,key,context.email,`${spool.hasUsage?"报废":"撤销误录"}：${reason}`)]);await recordAudit(context,"inventory_v2.spool.voided","material_spool",String(spoolId),{reason,hadUsage:Boolean(spool.hasUsage),removedGrams:spool.remaining});return Response.json({id:spoolId});
    }
    if(action==="bulkReceiveSpools"){
      const rows=Array.isArray(body.rows)?body.rows as Record<string,unknown>[]:[];if(!rows.length||rows.length>500)return fail("一次请导入 1 至 500 条实体卷");
      const catalogRows=(await db.prepare("SELECT id,catalog_code catalogCode FROM material_catalog_items WHERE organization_id=?").bind(org).all<{id:number;catalogCode:string}>()).results??[],locationRows=(await db.prepare("SELECT id,code FROM inventory_locations_v2 WHERE organization_id=? AND active=1").bind(org).all<{id:number;code:string}>()).results??[],catalogMap=new Map(catalogRows.map(row=>[row.catalogCode.toUpperCase(),row.id])),locationMap=new Map(locationRows.map(row=>[row.code.toUpperCase(),row.id])),seen=new Set<string>(),lotCatalog=new Map<string,number>(),normalized:{catalogId:number;spoolCode:string;lotNo:string;cost:number;net:number;tare:number;locationId:number;receivedAt:string;note:string;rowNumber:number}[]=[],errors:{rowNumber:number;errors:string[]}[]=[];
      for(let index=0;index<rows.length;index++){const raw=rows[index],rowErrors:string[]=[],catalogCode=text(raw.catalogCode,80).toUpperCase(),catalogId=catalogMap.get(catalogCode)||0,spoolCode=text(raw.spoolCode,80).toUpperCase(),lotNo=text(raw.lotNo,80).toUpperCase(),cost=Math.max(0,Math.round(Number(raw.costPerKg||0)*100)),net=grams(raw.netGrams),tare=grams(raw.tareGrams),locationCode=(text(raw.locationCode,80)||"MAIN").toUpperCase(),locationId=locationMap.get(locationCode)||0,receivedAt=text(raw.receivedAt,10)||now.slice(0,10),rowNumber=Number(raw.rowNumber)||index+2;if(!catalogId)rowErrors.push("耗材目录编码不存在");if(!spoolCode)rowErrors.push("缺少实体卷编号");if(seen.has(spoolCode))rowErrors.push("文件内实体卷编号重复");seen.add(spoolCode);if(!lotNo)rowErrors.push("缺少采购批次");if(net<=0)rowErrors.push("净重必须大于 0");if(tare<0)rowErrors.push("空盘重量无效");if(!locationId)rowErrors.push("仓位编码不存在");if(!/^\d{4}-\d{2}-\d{2}$/.test(receivedAt))rowErrors.push("入库日期格式无效");const previousCatalog=lotCatalog.get(lotNo);if(previousCatalog&&previousCatalog!==catalogId)rowErrors.push("同一采购批次不能对应多个耗材目录");lotCatalog.set(lotNo,catalogId);if(rowErrors.length)errors.push({rowNumber,errors:rowErrors});else normalized.push({catalogId,spoolCode,lotNo,cost,net,tare,locationId,receivedAt,note:text(raw.note,300),rowNumber})}
      if(errors.length)return Response.json({error:"批量导入校验失败",errors},{status:400});
      const existing=(await db.prepare(`SELECT spool_code spoolCode FROM material_spools WHERE organization_id=? AND spool_code IN (${normalized.map(()=>"?").join(",")})`).bind(org,...normalized.map(row=>row.spoolCode)).all<{spoolCode:string}>()).results??[];if(existing.length)return Response.json({error:"实体卷编号已存在",errors:existing.map(row=>({spoolCode:row.spoolCode}))},{status:409});
      const statements:ReturnType<typeof db.prepare>[]=[];for(const row of normalized){const token=crypto.randomUUID(),key=`bulk-receipt:${token}`;statements.push(db.prepare("INSERT INTO material_purchase_lots(organization_id,catalog_item_id,lot_no,unit_cost_cents_per_kg,received_at) VALUES(?,?,?,?,?) ON CONFLICT(organization_id,lot_no) DO UPDATE SET unit_cost_cents_per_kg=excluded.unit_cost_cents_per_kg,received_at=excluded.received_at").bind(org,row.catalogId,row.lotNo,row.cost,row.receivedAt),db.prepare("INSERT INTO material_spools(organization_id,spool_code,catalog_item_id,purchase_lot_id,current_location_id,state,initial_net_grams,remaining_net_grams,tare_grams,qr_token) SELECT ?,?,?,l.id,?,'sealed',?,?,?,? FROM material_purchase_lots l WHERE l.organization_id=? AND l.lot_no=?").bind(org,row.spoolCode,row.catalogId,row.locationId,row.net,row.net,row.tare,token,org,row.lotNo),db.prepare("INSERT INTO material_spool_movements(organization_id,spool_id,movement_type,to_location_id,net_grams_delta,idempotency_key,operator_email,note) SELECT ?,s.id,'receipt',?,?,?, ?,? FROM material_spools s WHERE s.organization_id=? AND s.spool_code=?").bind(org,row.locationId,row.net,key,context.email,row.note||`批量采购入库 ${row.lotNo}`,org,row.spoolCode))}
      await db.batch(statements);await recordAudit(context,"inventory_v2.bulk_receipt.created","material_spool","bulk",{count:normalized.length,lots:[...new Set(normalized.map(row=>row.lotNo))]});return Response.json({imported:normalized.length},{status:201});
    }
    ...
    if(action==="weigh"){
      ...
      const spool=await db.prepare("SELECT remaining_net_grams remaining,current_location_id locationId FROM material_spools WHERE id=? AND organization_id=?").bind(spoolId,org).first<{remaining:number;locationId:number}>();if(!spool)return fail("耗材卷不存在",404);const measured=Math.round((gross-tare)*1000)/1000,variance=Math.round((measured-spool.remaining)*1000)/1000,key=`weigh:${crypto.randomUUID()}`,requiresReview=Math.abs(variance)>2;
      if(requiresReview){const check=await db.prepare("INSERT INTO spool_weight_checks(organization_id,spool_id,gross_grams,tare_grams,measured_net_grams,book_net_grams,variance_grams,measured_by,measured_at) VALUES(?,?,?,?,?,?,?,?,?) RETURNING id").bind(org,spoolId,gross,tare,measured,spool.remaining,variance,context.email,now).first<{id:number}>();await db.batch([db.prepare("UPDATE material_spools SET tare_grams=?,last_gross_grams=?,last_weighed_at=?,updated_at=? WHERE id=? AND organization_id=?").bind(tare,gross,now,now,spoolId,org),db.prepare("INSERT INTO material_variance_cases(organization_id,spool_id,weight_check_id,book_net_grams,measured_net_grams,variance_grams,created_by) VALUES(?,?,?,?,?,?,?)").bind(org,spoolId,check!.id,spool.remaining,measured,variance,context.email)]);await recordAudit(context,"inventory_v2.variance.detected","material_spool",String(spoolId),{gross,tare,measured,variance,weightCheckId:check!.id});return Response.json({spoolId,measured,variance,requiresReview:true});}
      const movement=await db.prepare("INSERT INTO material_spool_movements(organization_id,spool_id,movement_type,from_location_id,to_location_id,net_grams_delta,idempotency_key,operator_email,note) VALUES(?,?,'adjust',?,?,?,?,?,?) RETURNING id").bind(org,spoolId,spool.locationId,spool.locationId,variance,key,context.email,"称重容差内自动校准").first<{id:number}>();
      ...
      await recordAudit(context,"inventory_v2.spool.weighed","material_spool",String(spoolId),{gross,tare,measured,variance});return Response.json({spoolId,measured,variance,requiresReview:false});
    }
