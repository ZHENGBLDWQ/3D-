...
export async function DELETE(request:Request){const denied=await requireApiAccess(true);if(denied)return denied;try{const id=Number(new URL(request.url).searchParams.get("id"));if(!id)return Response.json({error:"缺少设备标识"},{status:400});await getDb().delete(printerCommands).where(eq(printerCommands.printerId,id));await getDb().delete(printers).where(eq(printers.id,id));return Response.json({ok:true});}catch(error){return Response.json({error:error instanceof Error?error.message:"删除失败"},{status:500});}}
