...
import { printFiles, printerCommands, printers, spoolmanSpools } from "../../../db/schema";
...
export async function PATCH(request:Request){
  const denied=await requireApiAccess(true);if(denied)return denied;
  try{const p=await request.json() as {id?:number;status?:string;totalHours?:number;maintenanceDueAt?:string|null;action?:string;connectorType?:string;command?:string;fileId?:number};if(!p.id)return Response.json({error:"缺少设备标识"},{status:400});if(p.action==="rotateToken"){const token=`lt_${crypto.randomUUID().replaceAll("-","")}_${crypto.randomUUID().replaceAll("-","")}`;const [row]=await getDb().update(printers).set({connectorType:p.connectorType||"moonraker",connectorTokenHash:await sha256(token),connectionState:"等待代理连接"}).where(eq(printers.id,p.id)).returning();return Response.json({row,token});}if(p.action==="command"){if(!p.command||!["pause","resume","cancel","start"].includes(p.command))return Response.json({error:"不支持的设备命令"},{status:400});let payload="{}";if(p.command==="start"){if(!p.fileId)return Response.json({error:"请选择 G-code 文件"},{status:400});const [file]=await getDb().select().from(printFiles).where(eq(printFiles.id,p.fileId)).limit(1);if(!file||file.kind!=="G-code")return Response.json({error:"只能下发 G-code 文件"},{status:400});payload=JSON.stringify({fileId:file.id,filename:file.filename});}const [command]=await getDb().insert(printerCommands).values({printerId:p.id,command:p.command,payload}).returning();return Response.json({command},{status:202});}const [row]=await getDb().update(printers).set({status:p.status,totalHours:p.totalHours,maintenanceDueAt:p.maintenanceDueAt}).where(eq(printers.id,p.id)).returning();return Response.json({row});}catch(error){return Response.json({error:error instanceof Error?error.message:"更新失败"},{status:500});}
}

export async function DELETE(request:Request){const denied=await requireApiAccess(true);if(denied)return denied;try{const id=Number(new URL(request.url).searchParams.get("id"));if(!id)return Response.json({error:"缺少设备标识"},{status:400});await getDb().update(spoolmanSpools).set({syncedByPrinterId:null}).where(eq(spoolmanSpools.syncedByPrinterId,id));await getDb().delete(printerCommands).where(eq(printerCommands.printerId,id));await getDb().delete(printers).where(eq(printers.id,id));return Response.json({ok:true});}catch(error){return Response.json({error:error instanceof Error?error.message:"删除失败"},{status:500});}}
