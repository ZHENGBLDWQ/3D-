...
export const procurementRequestItems=sqliteTable("procurement_request_items",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),requestId:integer("request_id").notNull().references(()=>procurementRequests.id,{onDelete:"cascade"}),batchId:integer("batch_id").notNull().references(()=>materialBatches.id),requestedGrams:real("requested_grams").notNull(),suggestedGrams:real("suggested_grams").notNull().default(0),note:text("note").notNull().default("")});
...
export const materialCatalogItems=sqliteTable("material_catalog_items",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),catalogCode:text("catalog_code").notNull(),brand:text("brand").notNull().default(""),series:text("series").notNull().default(""),material:text("material").notNull(),colorName:text("color_name").notNull().default(""),colorNameEn:text("color_name_en").notNull().default(""),colorCode:text("color_code").notNull().default(""),colorHex:text("color_hex").notNull().default(""),densityGcm3:real("density_g_cm3").notNull().default(1.24),defaultNetGrams:real("default_net_grams").notNull().default(1000),defaultTareGrams:real("default_tare_grams").notNull().default(0),amsCompatibility:text("ams_compatibility").notNull().default("unknown"),tags:text("tags").notNull().default("[]"),legacyBatchId:integer("legacy_batch_id").references(()=>materialBatches.id,{onDelete:"set null"}),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const materialPurchaseLots=sqliteTable("material_purchase_lots",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),catalogItemId:integer("catalog_item_id").notNull().references(()=>materialCatalogItems.id),lotNo:text("lot_no").notNull(),supplierId:integer("supplier_id").references(()=>suppliers.id),purchaseOrderItemId:integer("purchase_order_item_id").references(()=>purchaseOrderItems.id),unitCostCentsPerKg:integer("unit_cost_cents_per_kg").notNull().default(0),receivedAt:text("received_at"),expiresAt:text("expires_at"),legacyBatchId:integer("legacy_batch_id").references(()=>materialBatches.id,{onDelete:"set null"}),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
