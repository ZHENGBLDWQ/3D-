...
export const supplierMaterialOffers=sqliteTable("supplier_material_offers",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),supplierId:integer("supplier_id").notNull().references(()=>suppliers.id,{onDelete:"cascade"}),catalogItemId:integer("catalog_item_id").notNull().references(()=>materialCatalogItems.id,{onDelete:"cascade"}),currency:text("currency").notNull().default("MYR"),unitPriceCentsPerSpool:integer("unit_price_cents_per_spool").notNull(),taxRateBps:integer("tax_rate_bps").notNull().default(0),freightCentsPerOrder:integer("freight_cents_per_order").notNull().default(0),minOrderSpools:integer("min_order_spools").notNull().default(1),leadTimeDays:integer("lead_time_days").notNull().default(0),validFrom:text("valid_from").notNull(),validUntil:text("valid_until"),status:text("status").notNull().default("active"),notes:text("notes").notNull().default(""),createdBy:text("created_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
...
export const replenishmentForecastSnapshots=sqliteTable("replenishment_forecast_snapshots",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),procurementRequestId:integer("procurement_request_id").notNull().references(()=>procurementRequests.id,{onDelete:"cascade"}),catalogItemId:integer("catalog_item_id").notNull().references(()=>materialCatalogItems.id),idempotencyKey:text("idempotency_key").notNull(),suggestedSpools:integer("suggested_spools").notNull(),adjustedSpools:integer("adjusted_spools").notNull(),forecastCostCents:integer("forecast_cost_cents").notNull().default(0),risk:text("risk").notNull(),confidence:text("confidence").notNull(),forecastJson:text("forecast_json").notNull(),adjustmentReason:text("adjustment_reason").notNull().default(""),createdBy:text("created_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const purchaseOrders=sqliteTable("purchase_orders",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),requestId:integer("request_id").notNull().references(()=>procurementRequests.id),supplierId:integer("supplier_id").notNull().references(()=>suppliers.id),selectedOfferId:integer("selected_offer_id"),purchaseNo:text("purchase_no").notNull(),status:text("status").notNull().default("approved"),costStatus:text("cost_status").notNull().default("legacy"),reconciliationStatus:text("reconciliation_status").notNull().default("not_submitted"),subtotalCents:integer("subtotal_cents").notNull().default(0),taxCents:integer("tax_cents").notNull().default(0),freightCents:integer("freight_cents").notNull().default(0),landedTotalCents:integer("landed_total_cents").notNull().default(0),costApprovedBy:text("cost_approved_by"),costApprovedAt:text("cost_approved_at"),orderedAt:text("ordered_at"),cancelledAt:text("cancelled_at"),createdBy:text("created_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const procurementMonthlyBudgets=sqliteTable("procurement_monthly_budgets",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),budgetMonth:text("budget_month").notNull(),budgetCents:integer("budget_cents").notNull(),warningBps:integer("warning_bps").notNull().default(8000),notes:text("notes"),createdBy:text("created_by").notNull(),updatedBy:text("updated_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const procurementBudgetOverrides=sqliteTable("procurement_budget_overrides",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),requestId:integer("request_id").notNull().references(()=>procurementRequests.id,{onDelete:"cascade"}),offerId:integer("offer_id").notNull().references(()=>supplierMaterialOffers.id,{onDelete:"cascade"}),budgetMonth:text("budget_month").notNull(),projectedCommitmentCents:integer("projected_commitment_cents").notNull(),budgetCents:integer("budget_cents").notNull(),reason:text("reason").notNull(),approvedBy:text("approved_by").notNull(),approvedAt:text("approved_at").notNull()});
export const purchaseOrderItems=sqliteTable("purchase_order_items",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),purchaseOrderId:integer("purchase_order_id").notNull().references(()=>purchaseOrders.id,{onDelete:"cascade"}),requestItemId:integer("request_item_id").notNull().references(()=>procurementRequestItems.id),batchId:integer("batch_id").notNull().references(()=>materialBatches.id),catalogItemId:integer("catalog_item_id").references(()=>materialCatalogItems.id),offerId:integer("offer_id"),orderedGrams:real("ordered_grams").notNull(),receivedGrams:real("received_grams").notNull().default(0),orderedSpools:integer("ordered_spools").notNull().default(0),receivedSpools:integer("received_spools").notNull().default(0),perSpoolNetGrams:real("per_spool_net_grams").notNull().default(0),unitCostPerKg:real("unit_cost_per_kg").notNull().default(0),unitCostCentsPerSpool:integer("unit_cost_cents_per_spool").notNull().default(0),landedCostCentsPerSpool:integer("landed_cost_cents_per_spool").notNull().default(0)});
...
export const materialPurchaseLots=sqliteTable("material_purchase_lots",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),catalogItemId:integer("catalog_item_id").notNull().references(()=>materialCatalogItems.id),lotNo:text("lot_no").notNull(),supplierId:integer("supplier_id").references(()=>suppliers.id),purchaseOrderItemId:integer("purchase_order_item_id").references(()=>purchaseOrderItems.id),goodsReceiptId:integer("goods_receipt_id"),unitCostCentsPerKg:integer("unit_cost_cents_per_kg").notNull().default(0),originalUnitCostCentsPerKg:integer("original_unit_cost_cents_per_kg"),reconciledInvoiceId:integer("reconciled_invoice_id"),receivedAt:text("received_at"),expiresAt:text("expires_at"),legacyBatchId:integer("legacy_batch_id").references(()=>materialBatches.id,{onDelete:"set null"}),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const supplierInvoices=sqliteTable("supplier_invoices",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),purchaseOrderId:integer("purchase_order_id").notNull().references(()=>purchaseOrders.id,{onDelete:"cascade"}),invoiceNo:text("invoice_no").notNull(),invoiceDate:text("invoice_date").notNull(),status:text("status").notNull().default("pending_review"),actualSubtotalCents:integer("actual_subtotal_cents").notNull(),actualTaxCents:integer("actual_tax_cents").notNull().default(0),actualFreightCents:integer("actual_freight_cents").notNull().default(0),actualTotalCents:integer("actual_total_cents").notNull(),approvedTotalCents:integer("approved_total_cents").notNull(),varianceCents:integer("variance_cents").notNull(),paymentTermsDays:integer("payment_terms_days").notNull().default(30),dueDate:text("due_date"),notes:text("notes").notNull().default(""),createdBy:text("created_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),reviewedBy:text("reviewed_by"),reviewedAt:text("reviewed_at"),reviewNote:text("review_note").notNull().default("")});
export const supplierPayments=sqliteTable("supplier_payments",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),supplierInvoiceId:integer("supplier_invoice_id").notNull().references(()=>supplierInvoices.id),paymentReference:text("payment_reference").notNull(),amountCents:integer("amount_cents").notNull(),method:text("method").notNull().default("bank_transfer"),paidAt:text("paid_at").notNull(),note:text("note").notNull().default(""),recordedBy:text("recorded_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const cashflowSettings=sqliteTable("cashflow_settings",{organizationId:integer("organization_id").primaryKey().references(()=>organizations.id,{onDelete:"cascade"}),availableCashCents:integer("available_cash_cents").notNull().default(0),minimumReserveCents:integer("minimum_reserve_cents").notNull().default(0),balanceAsOf:text("balance_as_of").notNull(),notes:text("notes").notNull().default(""),updatedBy:text("updated_by").notNull(),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const monthlyBusinessTargets=sqliteTable("monthly_business_targets",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),targetMonth:text("target_month").notNull(),revenueTargetCents:integer("revenue_target_cents").notNull().default(0),grossMarginTargetBps:integer("gross_margin_target_bps").notNull().default(0),operatingCashflowTargetCents:integer("operating_cashflow_target_cents").notNull().default(0),materialLossTargetBps:integer("material_loss_target_bps").notNull().default(0),utilizationTargetBps:integer("utilization_target_bps").notNull().default(0),notes:text("notes").notNull().default(""),createdBy:text("created_by").notNull(),updatedBy:text("updated_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),updatedAt:text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
export const materialCostAdjustments=sqliteTable("material_cost_adjustments",{id:integer("id").primaryKey({autoIncrement:true}),organizationId:integer("organization_id").notNull().references(()=>organizations.id,{onDelete:"cascade"}),supplierInvoiceId:integer("supplier_invoice_id").notNull().references(()=>supplierInvoices.id,{onDelete:"cascade"}),purchaseLotId:integer("purchase_lot_id").notNull().references(()=>materialPurchaseLots.id,{onDelete:"cascade"}),oldUnitCostCentsPerKg:integer("old_unit_cost_cents_per_kg").notNull(),newUnitCostCentsPerKg:integer("new_unit_cost_cents_per_kg").notNull(),status:text("status").notNull(),createdBy:text("created_by").notNull(),createdAt:text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)});
