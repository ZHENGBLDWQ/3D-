...
class BambuMonitorAdapter:
    ...
    monitor_only = True
    def __init__(self, binding_id: int, serial: str, emit: Callable[[dict], None], *, clock=time.time):
    ...
    def observe(self, report: dict) -> list[dict]:
        ...
        if status == "printing" and not self.session_key:
            seed = f"{self.serial}|{task_id}|{filename}|{now}"
        ...
        if session_key and status != self.last_status and status in {"printing","paused","completed","failed","cancelled"}:
            ...
            events.append(self._event("print.session", now, {**snapshot,"phase":phase,"externalSessionKey":session_key,"source":"bambu_studio"}, f"session:{session_key}:{phase}"))
